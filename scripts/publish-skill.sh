...
SKILL_NAME="hello-local"
WORK_DIR="$(mktemp -d /tmp/hello-local-skill-XXXXXX)"
trap 'rm -rf "$WORK_DIR"' EXIT
TS="$(date +%Y%m%d%H%M%S)"
RELEASE_PATH="hello-local/release"
...
echo "CLI_COMMAND=bash <(curl -fsSL https://skill.vyibc.com/${SKILL_NAME}.sh) --mode=hello --port=18091 --domain-name=hello-local --auto-domain-token=atd-76631b52126234666e0a12c6f45ac6d8"
